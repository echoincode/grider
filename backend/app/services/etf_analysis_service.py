...
from app.algorithms.grid.recovery_strategy import RecoveryGridStrategy
...
class ETFAnalysisService:
    ...
    def analyze_recovery_strategy(self, etf_code: str, existing_position: int,
        ...
        try:
            ...
            # 6. 计算回本网格配置
            grid_params = recovery_strategy.calculate_recovery_grid(
                current_price=current_price,
                atr_ratio=atr_ratio,
                risk_preference=risk_preference,
                adjustment_coefficient=adjustment_coefficient
            ...
            # 7. 生成策略分析依据
            strategy_rationale = self._generate_recovery_strategy_rationale(
                suitability_result, grid_params, risk_preference
            )
            ...
            logger.info(f"解套回本策略分析完成: {etf_code}, 适宜度评分{suitability_result['total_score']}, "
                       f"预期摊薄成本{grid_params['recovery_metrics']['expected_avg_cost']:.3f}")
            
            return complete_report
            
        except Exception as e:
            logger.error(f"解套回本策略分析失败: {etf_code}, {str(e)}")
            raise
    
    # [RECOVERY_STRATEGY] 新增：生成回本策略分析依据
    def _generate_recovery_strategy_rationale(self, suitability_result: Dict, 
                                              grid_params: Dict, risk_preference: str) -> Dict:
        """
        生成回本策略分析依据
        ...
        try:
            ...
            recovery_metrics = grid_params.get('recovery_metrics', {})
            
            parameter_logic = {
                'price_range': f"基于ATR比率{atr_analysis['current_atr_pct']:.2f}%和{risk_preference}频率偏好计算",
                'grid_count': f"非对称网格配置，下方{grid_params['grid_config']['region_counts']['lower']}个, "
                            f"中部{grid_params['grid_config']['region_counts']['middle']}个, "
                            f"上方{grid_params['grid_config']['region_counts']['upper']}个",
                'fund_allocation': f"原有持仓{grid_params['existing_position']}股，"
                                 f"底仓比例{grid_params['fund_allocation']['base_position_ratio']:.1%}",
                'recovery_target': f"预期摊薄成本{recovery_metrics.get('expected_avg_cost', 0):.3f}，"
                                 f"成本降低{recovery_metrics.get('cost_reduction_ratio', 0):.1%}"
            }
            
            profit_basis = {
                'cost_reduction': "基于非对称网格配置和资金分配估算",
                'recovery_time': "基于历史波动率和网格密度估算",
                'risk_control': "基于ATR波动率和最大可接受浮亏设定",
                'fund_allocation': "智能资金分配确保风险可控"
            }
            ...
            return {
                ...
                'profit_basis': profit_basis,
                'market_environment': {
                    'volatility': f"年化波动率{suitability_result['market_indicators']['volatility']:.1%}",
                    'trend_characteristic': suitability_result['evaluations']['market_characteristics']['market_type'],
                    'liquidity': suitability_result['evaluations']['liquidity']['level']
                }
            }
            ...
            logger.error(f"生成回本策略分析依据失败: {str(e)}")
            return {
                'strategy_type': 'recovery',
                'atr_advantages': [],
                'parameter_logic': {},
