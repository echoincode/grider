...
# [RECOVERY_STRATEGY] 新增：解套回本策略分析路由
...
def analyze_recovery_strategy():
    ...
    try:
        ...
        etf_code, country = determine_country(data.get('etfCode', '').strip())
        
        # 验证必需参数
        required_fields = ['existingPosition', 'existingCost', 'newCapital']
        ...
        etf_service = ETFAnalysisService(country=country)
        analysis_result = etf_service.analyze_recovery_strategy(
            etf_code=etf_code,
        ...
        if analysis_result['grid_strategy']['fund_allocation']['grid_fund_utilization_rate'] > 1 or analysis_result['grid_strategy']['fund_allocation']['grid_trading_amount'] < 0:
            return jsonify({
                'success': False,
                'message': '分析失败，标的价格较高，请增加新投入资金，降低交易频率'
            }), HTTP_BAD_REQUEST
        ...
        logger.error(f"回本策略数据解析失败: {str(e)}")
        return jsonify({
            'success': False,
            'message': '分析失败，数据解析错误'
        }), HTTP_INTERNAL_SERVER_ERROR
        ...
        logger.warning(f"回本策略参数验证错误: {str(e)}")
        return jsonify({
            'success': False,
            'message': str(e)
        }), HTTP_BAD_REQUEST
        ...
        logger.error(f"解套回本策略分析失败: {str(e)}")
        return jsonify({
            'success': False,
            'message': '分析失败，请稍后重试或检查标的数据是否充足'
        }), HTTP_INTERNAL_SERVER_ERROR
