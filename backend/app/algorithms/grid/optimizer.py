...
class GridOptimizer:
    ...
    # [RECOVERY_STRATEGY] 新增：解套回本策略步长计算
    def calculate_recovery_step_sizes(self, current_price: float, cost_price: float,
                                      atr_ratio: float, risk_preference: str,
        ...
        try:
            ...
            # 上方稀疏区：大步长，等待解套
            # 根据套牢程度动态调整上方步长系数
            drawdown_ratio = (cost_price - current_price) / cost_price if cost_price > 0 else 0
            ...
            # 上方区域起点：接近成本价的80%位置
            cost_distance = cost_price - current_price
            ...
            result = {
                ...
                'lower': {
                    ...
                    'end_price': round(middle_lower, 3)
                },
                'middle': {
                    'step_size': round(middle_step_size, 3),
                    'step_ratio': round(middle_step_ratio, 4),
                    'start_price': round(middle_lower, 3),
                    'end_price': round(middle_upper, 3)
                },
                'upper': {
                    'step_size': round(upper_step_size, 3),
                    'step_ratio': round(upper_step_ratio, 4),
                    'start_price': round(max(middle_upper, upper_start_price), 3)
                },
                'region_boundaries': {
            ...
            logger.info(f"回本策略步长计算完成: 下方{lower_step_ratio:.1%}, "
                       f"中部{middle_step_ratio:.1%}, 上方{upper_step_ratio:.1%}")
            
            return result
            
        except Exception as e:
            logger.error(f"回本策略步长计算失败: {str(e)}")
            # 返回默认步长配置
            default_step_ratio = 0.01
            return {
                'base_step_size': round(current_price * default_step_ratio, 3),
                'base_step_ratio': default_step_ratio,
                'lower': {'step_size': round(current_price * default_step_ratio * 0.5, 3), 'step_ratio': default_step_ratio * 0.5, 'end_price': current_price},
                'middle': {'step_size': round(current_price * default_step_ratio, 3), 'step_ratio': default_step_ratio, 'start_price': current_price, 'end_price': current_price * 1.05},
                'upper': {'step_size': round(current_price * default_step_ratio * 1.5, 3), 'step_ratio': default_step_ratio * 1.5, 'start_price': current_price * 1.05},
                'region_boundaries': {'middle_lower': current_price, 'middle_upper': current_price * 1.05, 'upper_start': current_price * 1.05}
            }
